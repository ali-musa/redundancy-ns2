...
for exp_num in sys.argv[1:]:
		...
		if file.startswith("ends"):
			...
			with open(read_dir+startsFilename, "r") as f1:
				for line in f1:
					lineList = line.split()
					if len(lineList)>0:
						if (int(lineList[1])>=19000 and int(lineList[1])<20000) or (int(lineList[1])>=39000 and int(lineList[1])<40000):
			...
			load = file.split("_")[0][5:]
			seed_value = file.split("_")[1].split(".")[0]
			totalQueueSize=[]
			for copy in range(int(k)):
				totalQueueSize.append((0,0))
			with open(read_dir+file, "r") as f:
				for line in f:
					lineSplit  = line.split("	")
					if len(lineSplit)>=4:
						if (int(lineSplit[0])>=19000 and int(lineSplit[0])<20000) or (int(lineSplit[0])>=39000 and int(lineSplit[0])<40000):
							totalQueueSize[int(lineSplit[2])] = (totalQueueSize[int(lineSplit[2])][0]+int(lineSplit[3]), totalQueueSize[int(lineSplit[2])][1]+1)
			for copy in range(int(k)):
				avg = 0
				if totalQueueSize[copy][1] !=0:
					avg = totalQueueSize[copy][0]/totalQueueSize[copy][1]
				with open(write_directory+"avgQ"+str(copy)+"_"+seed_value+".csv", 'a') as csvfile:
					csvfile.write(load)
					csvfile.write(",")
					csvfile.write(str(avg))
					csvfile.write("\n")
	...
	for copy in range(int(k)):
		calculateAverageOverSeeds("avgQ"+str(copy)+"_")
